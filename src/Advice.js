...
function Advice (){
  ...
  const previousItem = () => {setAdvice ((advice => {
     advice--;
     if (advice < 0) {
  ...
  const nextItem = () => {setAdvice (( advice => {
   advice++;
   ...
   return (
      <div className="advice">
         ...
         <div  className='buttons'>
            <button className="btn next" onClick={previousItem}><BiChevronLeft/></button>
             <img className='advice-image' src={image} alt="sale"/>
            <button className="btn prev" onClick={nextItem}><BiChevronRight/></button>
         </div>
